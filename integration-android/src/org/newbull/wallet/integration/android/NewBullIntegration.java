...
public final class NewBullIntegration {
    ...
    private static final String MIMETYPE_PAYMENTREQUEST = "application/newbull-paymentrequest"; // BIP 71
     ...
     * 
     * Warning: A success indication is no guarantee! To be on the safe side, you must drive your own NewBull
    ...
    /**
     * Put BIP70 payment message into result intent. Meant for usage by NewBull wallet applications.
     * 
     * @param result
     *            result intent
     * @param payment
    ...
    /**
     * Get BIP70 payment message from result intent. Meant for usage by applications initiating a NewBull
     * payment.
     * 
     * You can use the transactions contained in the payment to validate the payment. For this, you need your
     * own NewBull infrastructure though. There is no guarantee that the payment will ever confirm.
     * 
     * @param result
     *            result intent
    ...
    /**
     * Put transaction hash into result intent. Meant for usage by NewBull wallet applications.
     * 
     * @param result
     *            result intent
     * @param txHash
    ...
    /**
     * Get transaction hash from result intent. Meant for usage by applications initiating a NewBull payment.
     * 
     * You can use this hash to request the transaction from the NewBull network, in order to validate. For
     * this, you need your own NewBull infrastructure though. There is no guarantee that the transaction has
     * ever been broadcasted to the NewBull network.
     * 
     * @param result
     *            result intent
    ...
    private static Intent makeBitcoinUriIntent(final String address, final Long amount) {
        final StringBuilder uri = new StringBuilder("newbull:");
        if (address != null)
            uri.append(address);
        if (amount != null)
            uri.append("?amount=")
    ...
    private static void redirectToDownload(final Context context) {
        Toast.makeText(context, "No NewBull application found.\nPlease install NewBull Wallet.", Toast.LENGTH_LONG)
                .show();

        final Intent marketIntent = new Intent(Intent.ACTION_VIEW,
                Uri.parse("market://details?id=org.newbull.wallet"));
        final Intent binaryIntent = new Intent(Intent.ACTION_VIEW,
                Uri.parse("https://newbull.org/"));
